...
4. 先验证四个基准视角，再依次生成 row 9 和 row 10 的十六方向观察动画。
5. 仅在最终完整图集上执行一次去色键，并完成尺寸、v2、方向、连续性与独立视觉 QA。
...
4. Approve four cardinal views, then build look row 9 before row 10.
5. Despill the final atlas exactly once, then run size, v2, direction, continuity, and independent visual QA.
...
`SKILL.md` is the authoritative workflow. The reference files contain deterministic command recipes and failure routing; the bundled checker adds cross-slot component ownership QA.

`SKILL.md` 是权威工作流；参考文件提供确定性命令与失败分流；附带脚本用于检查跨帧组件归属。
