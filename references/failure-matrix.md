...
| Two accepted animation strips decode to pixel-identical RGBA content | Wrong artifact binding | Fail lane uniqueness, use the recorded job-to-output binding to identify the wrong lane, and replace only that complete lane | Trust different filenames, PNG byte hashes, or a busy contact sheet | Change the generation-output isolation strategy if the same cause recurs |
...
| Jumping is smaller because extraction fitted the full vertical arc into one cell | Scale/extraction coupling | Compare against the canonical scale profile and conditionally repair the complete row with the lowest pose anchored to the shared feet baseline | Normalize every frame separately or remove the vertical arc | Regenerate only when one uniform scale cannot fit without clipping |
| A grounded row differs from canonical body height by more than the profile tolerance | Cross-action scale drift | Run one conditional whole-row repair into a new directory, then repeat row QA | Quietly accept the pop or spend an image-generation call before deterministic repair | Regenerate only for anatomy-scale drift that one uniform transform cannot correct |
...
| Several validators pass but their full JSON outputs consume the parent context | Token/process waste | Aggregate them with `fast_qa_summary.py` and print its single-line result | Paste each report or ask the model to resummarize raw JSON | Open only the detailed report named by a failed summary label |
